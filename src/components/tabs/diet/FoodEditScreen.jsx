...
export default function FoodEditScreen({
  ...
  const recalculateFromQuantityAndMeasure = (qty, measureKey) => {
    ...
    const scale = totalGrams / Math.max(1, baseWeight);
    const cal = Math.round(baseCalories * scale);
    const p = Math.round(baseProtein * scale * 10) / 10;
    const c = Math.round(baseCarbs * scale * 10) / 10;
    const f = Math.round(baseFats * scale * 10) / 10;
    const fib = Math.round(baseFiber * scale * 10) / 10;
  ...
  const handleNetWeightChange = (newWeightVal) => {
    ...
    const cal = Math.round(baseCalories * scale);
    const p = Math.round(baseProtein * scale);
    const c = Math.round(baseCarbs * scale);
    const f = Math.round(baseFats * scale);
    const fib = Math.round(baseFiber * scale);
